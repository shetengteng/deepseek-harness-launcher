...
/// 不带缓存的元数据拉取。测试用：验证 wiremock 调用次数。
...
mod tests {
    ...
    /// 构造一份完整 fixture：3 个版本（0.1.0 latest, 0.1.0-rc.6, 0.0.9）。
    pub(crate) fn fixture_metadata_json() -> String {
        serde_json::json!({
    ...
    fn metadata_url_trims_trailing_slash() {
        ...
        assert_eq!(
            metadata_url("https://registry.npmmirror.com"),
            "https://registry.npmmirror.com/@deepseek-ai/dsh"
        );
    }

    #[test]
    fn manifest_url_includes_version() {
        assert_eq!(
            manifest_url("https://registry.npmjs.org", "0.1.0"),
            "https://registry.npmjs.org/@deepseek-ai/dsh/0.1.0"
        );
    }

    #[test]
    fn parse_metadata_extracts_latest() {
        let data: PackageMetadata = serde_json::from_str(&fixture_metadata_json()).expect("parse");
        assert_eq!(data.dist_tags.latest, "0.1.0");
        assert_eq!(
            data.dist_tags.others.get("next").and_then(|v| v.as_str()),
            Some("0.1.1-rc.1")
        );
    ...
    #[test]
    fn manifest_for_returns_parsed_fields() {
        let data: PackageMetadata = serde_json::from_str(&fixture_metadata_json()).expect("parse");
        let m = data.manifest_for("0.1.0").expect("manifest");
        assert_eq!(m.version, "0.1.0");
        assert_eq!(m.engines.node, ">=22.0.0");
        assert_eq!(m.dist.integrity, "sha512-cccccc==");
        assert!(m.dist.tarball.ends_with("/dsh-0.1.0.tgz"));
    }

    #[test]
    fn manifest_for_unknown_version_errors() {
        let data: PackageMetadata = serde_json::from_str(&fixture_metadata_json()).expect("parse");
        let err = data.manifest_for("9.9.9").unwrap_err();
        assert!(matches!(err, LauncherError::DshRegistry(_)));
    ...
    #[test]
    fn manifest_for_missing_engines_defaults_empty() {
        // 不含 engines 的版本应默认空字符串
            ...
            "versions": {
                "1.0.0": {
                    ...
                    "dist": {
                        ...
                        "tarball": "https://example.com/dsh-1.0.0.tgz"
                    }
                }
            }
        })
        .to_string();
        let data: PackageMetadata = serde_json::from_str(&raw).expect("parse");
        let m = data.manifest_for("1.0.0").expect("manifest");
        assert_eq!(m.engines.node, "");
    }

    #[test]
    fn wrong_package_name_serializes_but_caller_validates() {
        // serde 反序列化不会校验 name 字段值；由 fetch_package_metadata_with_client 校验
        let raw = serde_json::json!({
            "name": "@wrong/pkg",
            "dist-tags": { "latest": "1.0.0" },
            "versions": {}
        ...
        .to_string();
        let parsed: PackageMetadata = serde_json::from_str(&raw).expect("parse");
        assert_ne!(parsed.name, DSH_PACKAGE_NAME);
    }

    #[test]
    fn all_versions_returns_all_keys() {
        let data: PackageMetadata = serde_json::from_str(&fixture_metadata_json()).expect("parse");
        let mut v = data.all_versions();
        v.sort();
        assert_eq!(v, vec!["0.0.9", "0.1.0", "0.1.0-rc.6"]);
    }
    ...
    async fn cache_put_then_get_hits() {
        ...
        cache.put_metadata("https://r.example", data.clone()).await;
        assert_eq!(cache.len().await, 1);
        let got = cache.get_metadata("https://r.example").await.expect("hit");
        assert_eq!(got, data);
    }
    ...
    async fn cache_miss_returns_none() {
        ...
        assert!(cache.is_empty().await);
        assert!(cache
            .get_metadata("https://registry.npmjs.org")
            .await
            .is_none());
    }
    ...
    async fn cache_invalidate_clears_all() {
        let cache = RegistryCache::new();
        let data: PackageMetadata = serde_json::from_str(&fixture_metadata_json()).expect("parse");
        cache.put_metadata("a", data.clone()).await;
        cache.put_metadata("b", data).await;
    ...
    #[tokio::test]
    async fn cache_ttl_expires_after_5min() {
        let cache = RegistryCache::new();
        let data: PackageMetadata = serde_json::from_str(&fixture_metadata_json()).expect("parse");
        // 直接写入一个"5 分钟前"的条目，模拟过期
        {
            let mut map = cache.metadata.write().await;
            map.insert(
                "https://r.example".to_string(),
                (
                    data.clone(),
                    Instant::now() - REGISTRY_CACHE_TTL - Duration::from_secs(1),
                ),
            );
        }
        // 已过期，应该 miss
        assert!(cache.get_metadata("https://r.example").await.is_none());
    }
}
