...
| P1 | dsh 更新触发的 Node 升级确认与原子切换 | 已完成：确认、平台 archive、校验、指针切换，以及 dsh 失败/取消后的 Node 事务回滚已实现并有测试 |
...
- 2026-08-19 的本地基线为：Rust `cargo test --quiet` 通过 186 个 crate 测试和 4 个集成测试，前端 `pnpm test --run` 通过 56 个测试；`lint`、`vue-tsc` 和 `build` 均通过。
...
- [x] Node 切换与后续 dsh 安装已形成跨步骤事务；失败或取消时通过事务句柄恢复旧 Node 指针和 `state.json`。
...
- [x] 更新成功后提升 current、重启失败时回滚 known_good 的测试：覆盖 `commands/host.rs` 和 `dsh/version/tests.rs`。
...
- [x] 后端：
  - `install_dsh_command` 在首启复用冻结计划；更新场景只接收提示或设置页已展示且经确认的精确版本，并重新校验该版本 manifest
  - 启动时的版本检查只读取元数据，失败不影响 dsh 启动；不引入定时任务或数据库
  - `state.json` 增加 `last_notified`，避免同一版本在每次启动时重复打扰
  - 安装成功后立即提升为 current；重启失败则回滚 known_good
  - 安装失败清理目标目录，保留当前版本；Node 不兼容时不写 current，并返回可操作错误
- [x] 前端：
...
- [x] 测试与验收：
  - Rust：精确版本选择、已提示版本过滤、Node 不兼容时不提升 current、回滚
  - Vue：展示/刷新最新版本、同版本禁用更新、精确版本安装、网络错误时保留当前版本
  - 手动：registry 发布新版本后，启动时只提示；用户点击更新后安装、切换并重启

### M3.5 既有升级功能的收敛

PR-015 与 PR-016 的版本范围输入、历史版本选择、自动下载和复杂升级设置不再是产品功能。PR-020c 以轻量版本检查、右侧更新提示、用户主动安装、安装进度、版本指针与回滚取代它们。用户点击更新即确认安装、切换并重启，不再增加第二次重启确认。
...
- 用户点击更新后，安装完成立即切换并重启；启动失败回滚到 `known_good`
...
- [x] 在崩溃弹窗中提供“退出应用”操作，并复用托盘优雅退出命令。
...
- [x] Node 升级成功而后续 dsh 安装失败或取消时，回滚 Node 指针与 `state.json`，形成跨步骤事务。
...
- [ ] 增加真实原生托盘与 Host 生命周期联动的桌面 E2E；当前已补齐全生命周期状态序列与平台图标资源测试。
...
- Node 安装已有指针恢复和取消清理测试，覆盖平台 archive、二进制版本校验、`VERSION` 原子替换与回滚。
- 2026-08-19 本地基线：`cargo test --quiet` 通过 186 个 crate 测试和 4 个集成测试；格式检查通过。

### 9.2 已有前端测试

- Vitest 覆盖首启、启动遮罩、更新流程和设置页的关键交互；托盘/Host 原生联动尚无前端桌面 E2E。
- 2026-08-19 本地基线：`pnpm test --run` 通过 56 个测试；`pnpm lint && pnpm build` 通过，`pnpm build` 已包含 `vue-tsc --noEmit`。

### 9.3 测试待办

- [x] 为 PR-020b/PR-020c 增加“展示版本即安装版本”、立即切换重启、Node 不兼容、取消和同版本重试的 Rust/Vue 测试。
- [x] 为 dsh 文件日志和诊断导出最近 3 份日志增加测试。
- [x] 补充 mock 子进程的 Host 生命周期集成测试：就绪、超时、意外退出、重启和关闭（Unix runner）。
- [x] 补充 Node 升级后 dsh 安装失败/取消的跨步骤回滚测试，并实现对应事务回滚。
- [ ] 补充真实原生托盘状态与 Host 生命周期联动的桌面集成测试；当前已覆盖状态序列、状态文案和平台图标资源。
...
- ✅ PR-017 [M4] 崩溃计数 + 自动重启 + 弹窗（含“退出应用”并复用托盘优雅退出）
- ✅ PR-018 [M4] Node 升级确认、安装、切换与跨步骤回滚；mock Host/Node 回滚测试已补（真实桌面 E2E 待补）
- ✅ PR-019 [M4] 错误文案、诊断导出、dsh 子进程日志落盘与“最近 3 份”筛选
- ✅ PR-020 [M4] 磁盘/网络错误识别（`node/disk.rs` 200MB 下载前检查 + 下载错误文案区分网络/磁盘不足）
- 🚧 PR-020a [M4] 系统托盘与窗口生命周期（状态同步、事件桥接、退出时 Host shutdown 和平台图标已完成；真实桌面联动测试待补，详见 §M4.5）
