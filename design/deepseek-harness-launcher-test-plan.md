...
**测试目标**：semver 解析、engines 校验、版本选择。
...
**测试目标**：check/prepare/run 三阶段、试运行、回滚触发。

| 文件 | 用例 | 期望 |
|---|---|---|
| `upgrade.rs` | `check_for_upgrade` 有新版本 | 返回新版本号 |
| `upgrade.rs` | `check_for_upgrade` 无新版本 | `Ok(None)` |
| `upgrade.rs` | `prepare_upgrade` 下载 + 校验 | 临时目录就绪 |
| `upgrade.rs` | 试运行成功 | `known_good` 更新 |
| `upgrade.rs` | 试运行崩溃 | 自动回滚到 `known_good` |
| `upgrade.rs` | 并发升级请求 | 第二个返回 `UpgradeInProgress` |

**集成测试**：`tests/upgrade.rs` 用 mock registry + mock tarball。
...
| 文件 | 用例 | 期望 |
|---|---|---|
| `UpgradeDialog.vue` | 有新版本 | 显示版本号 + 升级按钮 |
| `UpgradeDialog.vue` | 升级中 | 显示进度，禁用关闭 |
| `UpgradeDialog.vue` | 升级成功 | 关闭对话框，显示成功提示 |
| `UpgradeDialog.vue` | 升级失败 | 显示错误 + 重试按钮 |
| `SettingsPage.vue` | 当前版本展示 | 显示 `current` 版本号 |
| `SettingsPage.vue` | 手动检查更新 | 触发 `check_for_upgrade` |
